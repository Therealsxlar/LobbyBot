...
GithubVersion = requests.get("https://raw.githubusercontent.com/Therealsxlar/LobbyBot/master/Settings.json").json()["BotVersion"]
if GithubVersion != Settings["BotVersion"]:
    Setting = Setting.replace(Settings["BotVersion"],GithubVersion)
    with open("Settings.json","w+") as f:
        f.write(Setting)
    r = requests.get("https://github.com/Therealsxlar/LobbyBot/archive/master.zip")
    z = zipfile.ZipFile(io.BytesIO(r.content))
    for fileName in z.namelist():
        if not "Settings.json" in fileName:
            z.extract(fileName, '')
    os.system("move LobbyBot-master\*")
    os.system("rmdir LobbyBot-master")
    os.system("Start.bat")
else:
